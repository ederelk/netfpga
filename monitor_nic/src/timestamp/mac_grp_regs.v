...
module mac_grp_regs
   ...
   ( input                                mac_grp_reg_req,
     ...
     input                                rx_pkt_pulled,
     input  [31:0]                        out_timestamp_bad_low,
     input  [31:0]                        out_timestamp_bad_high,
     input                                out_timestamp_bad_valid,
   ...
   // ------------- Internal parameters --------------
   localparam NUM_REGS_USED         = 15; /* don't forget to update this when adding regs */
   ...
   reg [31:0]                          out_timestamp_bad_low_delta;
   reg [31:0]                          out_timestamp_bad_high_delta;
   ...
   always @(posedge clk) begin
      ...
      if (reset)
         out_timestamp_bad_low_delta <= 'h0;
      else if(!new_reg_req && reg_cnt == `MAC_GRP_TIMESTAMP_BAD_LOW)
        out_timestamp_bad_low_delta <= out_timestamp_bad_valid ? out_timestamp_bad_low : 'h0;
      else if (out_timestamp_bad_valid)
         out_timestamp_bad_low_delta <= out_timestamp_bad_low_delta;
       
      if (reset)
       out_timestamp_bad_high_delta <= 'h0;
      else if (!new_reg_req && reg_cnt == `MAC_GRP_TIMESTAMP_BAD_HIGH)
       out_timestamp_bad_high_delta <= out_timestamp_bad_valid ? out_timestamp_bad_high : 'h0;
      else if (out_timestamp_bad_valid)
       out_timestamp_bad_high_delta <= out_timestamp_bad_high_delta;
            ...
            NORMAL : begin
               if(new_reg_req) begin // read request
                  ...
                  case (reg_cnt)
                     ...
                     `MAC_GRP_TIMESTAMP_BAD_LOW :              delta = 'h1ffe;
                     `MAC_GRP_TIMESTAMP_BAD_HIGH :             delta = 'h1fff;
                     default :                                 delta = 0;
                  endcase // case (reg_cnt)
                 
                  if(delta=='h1ffe)
                   reg_file_in = out_timestamp_bad_low_delta;
                  else if (delta=='h1fff)
                   reg_file_in = out_timestamp_bad_high_delta;
                  else 
